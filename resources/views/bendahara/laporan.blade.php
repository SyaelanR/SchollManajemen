...
<body class="bg-gray-50 text-gray-800 flex min-h-screen">
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div id="pendapatan-card" class="bg-indigo-50 p-6 rounded-xl shadow-lg border-l-4 border-indigo-500">
                <!-- Data will be rendered by JS -->
            </div>
            <div id="beban-card" class="bg-rose-50 p-6 rounded-xl shadow-lg border-l-4 border-rose-500">
                <!-- Data will be rendered by JS -->
            </div>
            <div id="laba-bersih-card" class="bg-green-50 p-6 rounded-xl shadow-lg border-l-4 border-green-500">
                <!-- Data will be rendered by JS -->
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <a href="{{ route('bendahara.laporan.neraca') }}" class="block bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300">
            ...
            <a href="{{ route('bendahara.laporan.laba_rugi') }}" class="block bg-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300">
                <div class="flex items-center justify-between mb-4">
    ...
    <script>
        ...
        // --- Perhitungan dan Rendering Ringkasan Keuangan ---
        let pendapatan = 0;
        let beban = 0;

        journals.forEach(journal => {
            if (journal.kreditAccount === 'Penjualan') {
                pendapatan += journal.nominal;
            }
            if (journal.debitAccount.startsWith('Beban')) {
                beban += journal.nominal;
            }
        });

        const labaBersih = pendapatan - beban;

        const pendapatanCard = document.getElementById('pendapatan-card');
        const bebanCard = document.getElementById('beban-card');
        const labaBersihCard = document.getElementById('laba-bersih-card');

        pendapatanCard.innerHTML = `<span class="font-semibold text-gray-500">Pendapatan</span><div class="text-3xl font-bold">${formatRupiah(pendapatan)}</div>`;
        bebanCard.innerHTML = `<span class="font-semibold text-gray-500">Beban</span><div class="text-3xl font-bold">${formatRupiah(beban)}</div>`;
        
        // Atur warna teks berdasarkan laba bersih
        const labaBersihColor = labaBersih >= 0 ? 'text-green-600' : 'text-red-600';
        labaBersihCard.innerHTML = `<span class="font-semibold text-gray-500">Laba Bersih</span><div class="text-3xl font-bold ${labaBersihColor}">${formatRupiah(labaBersih)}</div>`;

        // --- Rendering Jurnal Terbaru ---
        const latestJournals = journals.slice(0, 5); // Ambil 5 jurnal terbaru
