...
<div class="max-w-xl mx-auto bg-white rounded-xl shadow-md p-8">
    <h2 class="text-2xl font-bold mb-6 text-gray-800">Tambah Pemasukan</h2>

    <form action="{{ route('keuangan.storePemasukan') }}" method="POST" class="space-y-4" autocomplete="off">
    @csrf
    <div>
    ...
    <div>
        ...
        <input type="text" name="deskripsi" class="w-full border rounded px-3 py-2" placeholder="Contoh: SPP Kelas X" required autocomplete="off">
    </div>
    <div>
        <label class="block text-gray-700 mb-1">Jumlah (Rp)</label>
        <input type="number" name="jumlah" class="w-full border rounded px-3 py-2" placeholder="Contoh: 5000000" required autocomplete="off">
    </div>
