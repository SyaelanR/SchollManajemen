...
<body class="bg-gray-50 text-gray-800 flex min-h-screen">
    <aside class="w-64 bg-gray-900 text-gray-100 p-6 flex flex-col items-center">
        ...
        <nav class="w-full">
            <ul>
                ...
                <li class="mb-4">
                    <a href="/pelanggan" class="flex items-center p-3 rounded-lg bg-gray-800 text-indigo-400 font-semibold transition-colors duration-200">
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-4xl font-extrabold text-gray-900">Edit Pelanggan</h1>
            <a href="/pelanggan" class="flex items-center px-4 py-2 bg-indigo-600 text-white font-semibold rounded-md shadow-md hover:bg-indigo-700 transition-colors duration-200">
        ...
        <div class="bg-white p-6 rounded-xl shadow-md mb-8">
            <h2 class="text-xl font-bold mb-4 text-gray-800">Formulir Edit Pelanggan</h2>
            <form id="editPelangganForm" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="nama_pelanggan" class="block text-sm font-medium text-gray-700">Nama Pelanggan</label>
                        <input type="text" name="nama_pelanggan" id="nama_pelanggan" value="PT Sumber Makmur" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    </div>
                    <div>
                        <label for="kontak" class="block text-sm font-medium text-gray-700">Kontak</label>
                        <input type="text" name="kontak" id="kontak" value="08123456789" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" name="email" id="email" value="pt.sumbermakmur@example.com" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    </div>
                    <div>
                        <label for="kelas_santri" class="block text-sm font-medium text-gray-700">Kelas/Relasi Santri</label>
                        <input type="text" name="kelas_santri" id="kelas_santri" value="Kelas 10" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    </div>
                </div>
                <div class="mt-6 flex justify-end space-x-4">
                    <a href="/pelanggan" class="px-4 py-2 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 transition-colors text-center font-semibold">Batal</a>
                    <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors text-center font-semibold">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </main>
    ...
    <div id="berhasil-modal" class="modal-overlay">
        ...
        <div class="modal-content bg-white p-8 rounded-xl shadow-2xl max-w-sm w-full relative z-10 text-center">
            ...
            <div class="flex flex-col items-center">
                ...
                <p class="text-gray-600" id="success-message">Perubahan pelanggan berhasil disimpan.</p>
                <div class="mt-6">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const editPelangganForm = document.getElementById('editPelangganForm');
            const successModalMessage = document.getElementById('success-message');
            const closeSuccessBtn = document.getElementById('closeSuccessBtn');

            editPelangganForm.addEventListener('submit', (event) => {
                event.preventDefault(); // Mencegah form untuk redirect secara default
                
                // Menampilkan modal sukses
                successModalMessage.textContent = 'Perubahan pelanggan berhasil disimpan.';
                window.location.hash = 'berhasil-modal';

                // Menunggu 2 detik, lalu menutup modal dan mengalihkan halaman
                setTimeout(() => {
                    window.location.href = '/pelanggan';
                }, 2000);
            });

            // Handle closing the success modal manually
